...
class Compiler:
    ...
    def shrink_stmt(self, s: stmt) -> stmt:
        match s:
            ...
            case Assign([lhs], rhs):
                return Assign([self.shrink_exp(lhs)], self.shrink_exp(rhs))
            case Collect(_):
                return s
            case _:
    ...
    def expose_allocation_expr(self, e: expr) -> expr:
        match e:
            ...
            case Tuple(exprs, Load()):
                ...
                assignments: list[stmt] = [Assign([Name(generate_name('init'))], nexpr) for nexpr in nexprs]
    ...
    def expose_allocation_stmt(self, s: stmt) -> stmt:
        match s:
            ...
            case While(test, stmts, []):
                return While(
               		self.expose_allocation_expr(test), 
                 	self.expose_allocation_stmts(stmts),
                  	[])
            case _:
    ...
    def select_assign(self, s: stmt) -> list[Instr]:
        ...
        match s:
            ...
            case Assign([Name(var)], Allocate(n, t)):
                ...
                fwd    = 0b1        #  1 bit for forwarding (GC)
